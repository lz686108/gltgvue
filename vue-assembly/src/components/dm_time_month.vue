<template>
  <div class="dm_header">
    <div class="dm_header1">
      ...
      <div class="dm_day dm_day1" @click="day(1)">前一年</div>
      <div class="dm_day" @click="day(2)">当 年</div>
      <div class="dm_day" @click="day(3)">后一年</div>
    </div>
  </div>
</template>
<script>
    ...
    day(e) {
      if (e == 1) {
        this.time = this.$api.timeFormat(
            new Date(this.time) * 1 - 86400 * 1000 * 30 * 12,
            "month"
        );
      }
      if (e == 2) {
      ...
      if (e == 3) {
        this.time = this.$api.timeFormat(
            new Date(this.time) * 1 + 86400 * 1000 * 31 * 12,
            "month"
        );
      }
      this.$emit("dmclick", this.time);
